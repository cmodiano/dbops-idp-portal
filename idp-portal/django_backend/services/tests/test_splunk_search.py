...
class TestSearchPlatformLogs:
    ...
    async def test_success_returns_content(self, splunk_service):
        ...
        # Verify the search job creation request
        mock_client.post.assert_called_once()
        call_args = mock_client.post.call_args
        assert call_args[0][0] == "https://splunk.example.com:8089/services/search/jobs"
        assert 'search sourcetype="idp:platform_log" execution_id=42' in call_args[1]["data"]["search"]
