...
# === Database Migration V000 / V002 Tests (Story 2.1, AC #3; Story 2.20: Flyway naming, identity) ===

V000_FILE = "V000__create_schema_version.sql"
V002_FILE = "V002__create_actions_catalog.sql"


def test_v000_migration_exists():
    """V000 migration file exists (Flyway convention V000__desc.sql)."""
    migration = PROJECT_ROOT / "database" / "migrations" / V000_FILE
    assert migration.is_file(), "V000 migration file should exist"


def test_v000_creates_schema_version():
    """V000 creates SCHEMA_VERSION table (dropped later by V015)."""
    migration = PROJECT_ROOT / "database" / "migrations" / V000_FILE
...
def test_v002_migration_exists():
    """AC #3: V002 migration file exists (Flyway convention)."""
    migration = PROJECT_ROOT / "database" / "migrations" / V002_FILE
...
def test_v002_migration_uses_identity_column():
    """Story 2.20: V002 uses identity column for ID (no sequence)."""
    migration = PROJECT_ROOT / "database" / "migrations" / V002_FILE
    content = migration.read_text()
    assert "GENERATED ALWAYS AS IDENTITY" in content or "GENERATED BY DEFAULT AS IDENTITY" in content
...
def test_v002_migration_no_schema_version_merge():
    """Story 2.20: V002 does not update SCHEMA_VERSION (Flyway uses flyway_schema_history)."""
    migration = PROJECT_ROOT / "database" / "migrations" / V002_FILE
    content = migration.read_text()
    assert "MERGE INTO SCHEMA_VERSION" not in content


# === Database Migration V003 Tests (Story 2.2, AC #4; Story 2.20: Flyway naming) ===

V003_FILE = "V003__add_execution_steps.sql"


def test_v003_migration_exists():
    """AC #4: V003 migration file exists (Flyway convention)."""
    migration = PROJECT_ROOT / "database" / "migrations" / V003_FILE
...
def test_v003_migration_no_schema_version_merge():
    """Story 2.20: V003 does not update SCHEMA_VERSION (Flyway uses flyway_schema_history)."""
    migration = PROJECT_ROOT / "database" / "migrations" / V003_FILE
    content = migration.read_text()
    assert "MERGE INTO SCHEMA_VERSION" not in content
