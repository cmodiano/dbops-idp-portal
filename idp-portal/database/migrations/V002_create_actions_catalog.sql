...
CREATE SEQUENCE SEQ_ACTIONS_CATALOG START WITH 1 INCREMENT BY 1;

-- Main catalog table
CREATE TABLE ACTIONS_CATALOG (
    ID                  NUMBER DEFAULT SEQ_ACTIONS_CATALOG.NEXTVAL PRIMARY KEY,
...
-- Task 1.5: Update SCHEMA_VERSION with V002 (idempotent MERGE)
MERGE INTO SCHEMA_VERSION D
USING (SELECT 'V002' AS V FROM DUAL) S ON (D.VERSION = S.V)
